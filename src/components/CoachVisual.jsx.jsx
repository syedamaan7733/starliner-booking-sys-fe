...
function CoachVisual() {
  const seats = useSelector((store) => store.booking.seats);
  // console.log(seats);
  return (
    <div className=" bg-gray-100 shadow-lg p-6 rounded-2xl items-center text-center max-w-200">
      ...
      <div className="flex pt-3 justify-around items-center">
        {/* Badges */}
        <Badge variant={"outline"} className="bg-pink-100 border-primary">Booked</Badge>
        <Badge variant={"outline"} className='bg-gray-400 border-gray-400 text-stone-50'>Available</Badge>
      </div>
    </div>
  );
}
